...
contract PreSale is Ownable2Step, ReentrancyGuardTransient {
    ...
    event NodeFundsWalletUpdated(
        address oldNodeFundsWallet,
        address newNodeFundsWallet
    );

    /// @dev Emitted when address of miner funds wallet is updated
    event MinerFundsWalletUpdated(
        address oldMinerFundsWallet,
    ...
    event TokenRegistryUpdated(
        ITokenRegistry oldTokenRegistry,
        ITokenRegistry newTokenRegistry
    );

    /// @dev Emitted when miner price accretion percentage is updated
    event PriceAccretionPercentageUpdated(
        uint256 oldPriceAccretionPercent,
        uint256 newPriceAccretionPercent
    );
    ...
    /// @notice Thrown when discount percentage is greater than fifty
    ...
    function purchaseNodeNFT(
        ...
        uint256 purchaseAmount = (quantity *
            nodeNFTPrice *
    ...
    /// @param discountPercentagePPM The discount percentage in PPM
    /// @param referenceNormalizationFactor The normalization factor
        ...
        if (raised >= ONE_MILLION_DOLLAR) {
            ...
            for (uint256 i; i < quantityLength; ++i) {
                for (uint256 j; j < repetitions; ++j) {
                    minerNFTPrices[i] +=
                        (minerNFTPrices[i] * priceAccretionPercentagePPM) /
                        PPM;
                }
            }
        }
    ...
    function updateTokenRegistry(
        ...
        emit TokenRegistryUpdated({
            oldTokenRegistry: oldTokenRegistry,
            newTokenRegistry: newTokenRegistry
        });
    ...
    function updateMinerPriceAccretionPercent(
        uint256 newPriceAccretionPercent
    ) external onlyOwner {
    ...
    function updateNodeFundsWallet(
        address newNodeFundsWallet
    ) external checkAddressZero(newNodeFundsWallet) onlyOwner {
        ...
        emit NodeFundsWalletUpdated({
            oldNodeFundsWallet: oldNodeFundsWallet,
            newNodeFundsWallet: newNodeFundsWallet
        });
    ...
    function _transferAndUpdateCommissions(
        ...
        for (uint256 i; i < toLength; ++i) {
            ...
            claimInfo[i] = ClaimInfo({
                token: token,
                amount: (amount * percentage) / PPM
            });
        }
