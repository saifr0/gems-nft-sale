...
contract PreSaleTest is Test {
    ...
    function setUp() public {
        ...
        TokenRegistry.PriceFeedData[]
            memory priceFeeds = new TokenRegistry.PriceFeedData[](3);
    ...
    function testNodePurchaseWithETH() external {
        ...
        uint256 latestPriceETH = tokenRegistryContract
            .getLatestPrice(IERC20(ETH))
            .latestPrice;
        uint8 nfETH = tokenRegistryContract
            .getLatestPrice(IERC20(ETH))
            .normalizationFactor;
        ...
        //node buying
        vm.startPrank(user);
        preSale.purchaseNodeNFT{value: 1 ether}(
            ETH,
            quantities,
            price,
            deadline,
        ...
        assertEq(
            preSale.nodeFundsWallet().balance,
            expectedNodeWalletFunds,
            "Node Wallet Funds"
        );
        assertEq(
            user.balance,
            expectedUserPayment,
            "User balance after payment"
        );
    ...
    function testNodePurchaseWithUSDT() external {
        ...
        //node walllet balance assertion
        assertEq(
            USDT.balanceOf(preSale.nodeFundsWallet()),
            expectedNodeWalletFunds,
            "Node Wallet Funds"
        );
    ...
    function testNodePurchaseWithGEMS() external {
        ...
        //node walllet balance assertion
        assertEq(
            GEMS.balanceOf(preSale.nodeFundsWallet()),
            expectedNodeWalletFunds,
            "Node Wallet Funds"
        );
    ...
    function testMinerPurchaseWithETH() external {
        ...
        //miner buying
        vm.startPrank(user);
        preSale.purchaseMinerNFT{value: 0.1 ether}(
            ETH,
            price,
            deadline,
            quantities,
        ...
        assertEq(
            preSale.minerFundsWallet().balance,
            expectedMinerWalletFunds,
            "Miner Wallet Funds"
        );
        assertEq(
            user.balance,
    ...
    function testMinerPurchaseWithUSDT() external {
        ...
        //getting USDT latest price and normalization factor
        uint256 latestPriceUSDT = tokenRegistryContract
            .getLatestPrice(IERC20(USDT))
            .latestPrice;
        uint8 nfUSDT = tokenRegistryContract
            .getLatestPrice(IERC20(USDT))
            .normalizationFactor;
        ...
        //miner and user walllet balance assertion
        assertEq(
            USDT.balanceOf(preSale.minerFundsWallet()),
            expectedMinerWalletFunds,
            "Miner Wallet Funds"
        );
    ...
    function testMinerPurchaseWithGEMS() external {
        ...
        //miner and user walllet balance assertion
        assertEq(
            GEMS.balanceOf(preSale.minerFundsWallet()),
            expectedMinerWalletFunds,
            "Miner Wallet Funds"
        );
    ...
    function testMinerDiscountPurchaseWithETH() external {
        ...
        for (uint256 i = 0; i < leaders.length; ++i) {
            previousLeaderClaims[i] = claimsContract.pendingClaims(
                leaders[i],
                round,
                IERC20(ETH)
            );
        ...
        prices =
            (((prices * (10 ** nfETH)) / latestPriceETH) *
                discount_percentage_ppm) /
            PPM;
        ...
        assertEq(
            address(claimsContract).balance,
            expectedClaimsFunds - prevClaimsFunds,
            "claims"
        );
        assertEq(
            user.balance,
        ...
        for (uint256 i = 0; i < leaders.length; ++i) {
            ...
            assertEq(
                claimsContract.pendingClaims(
                    leaders[i],
                    claimsContract.currentWeek(),
                    IERC20(ETH)
        ...
        assertEq(
            expectedTotalPercentage,
            leaderPercentageAmount,
            "leader percentage contract"
        );
    ...
    function testMinerDiscountPurchaseWithUSDT() external {
        ...
        prices =
            (((prices * (10 ** nfUSDT)) / latestPriceUSDT) *
                discount_percentage_ppm) /
            PPM;
        ...
        assertEq(
            USDT.balanceOf(address(claimsContract)),
            expectedClaimsFunds - prevClaimsFunds,
            "claims"
        );
    ...
    function testMinerDiscountPurchaseWithGEMS() external {
        ...
        //GEMS price will come from backend
        prices =
            (((prices * (10 ** nf)) / price) * discount_percentage_ppm) /
            PPM;
        ...
        assertEq(
            GEMS.balanceOf(address(claimsContract)),
            expectedClaimsFunds - prevClaimsFunds,
            "claims"
        );
    ...
    function testLeaderClaims() external {
        ...
        claimsContract.revokeLeaderClaim(
            revokeLeader,
            claimsContract.currentWeek(),
            revokeToken,
            revokeAmount
        ...
        assertEq(
            expectedTotalPercentage + percentages[0],
            leaderPercentageAmount,
            "leader percentage contract"
        );
